...
class ModelPansharpening(ModelBase):
    """Train with pixel loss"""

    def __init__(self, opt):
        super(ModelPansharpening, self).__init__(opt)
        # ------------------------------------
        # define network
        # ------------------------------------
    ...
    def define_optimizer(self):
        ...
        if self.opt_train['G_optimizer_type'] == 'adam':
            ...
            self.G_optimizer = AdamW(G_optim_params, lr=self.opt_train['G_optimizer_lr'],
                                     betas=self.opt_train['G_optimizer_betas'],
                                     eps=self.opt_train['G_optimizer_eps'],
                                     weight_decay=self.opt_train['G_optimizer_wd'])
        elif self.opt_train['G_optimizer_type'] == 'lion':
            self.G_optimizer = Lion(G_optim_params, lr=self.opt_train['G_optimizer_lr'],
                                    betas=self.opt_train['G_optimizer_betas'],
                                    weight_decay=self.opt_train['G_optimizer_wd'])
        else:
    ...
    def netG_forward(self):
        ...
        self.sr = self.netG(self.ms, self.pan)
    ...
    def optimize_parameters(self, current_step):
        ...
        self.netG_forward()
        G_loss = self.G_lossfn_weight * self.G_lossfn(self.sr, self.gt)
        G_loss.backward()
