...
def main(json_path='options/train_mdcun.json'):
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0:
                ...
                for test_data in test_loader:
                    idx += 1
                    image_name_ext = os.path.basename(test_data['gt_path'][0])
                    ...
                    gt_img = util.tensor2uint(visuals['sr'])
                    sr_img = util.tensor2uint(visuals['gt'])

                    # -----------------------
                    # save estimated image E
                    # -----------------------
                    save_img_path = os.path.join(img_dir, '{:s}_{:d}.png'.format(img_name, current_step))
                    util.imsave(gt_img, save_img_path)

                    # -----------------------
                    # calculate PSNR
                    # -----------------------
                    current_psnr = util.calculate_psnr(sr_img, gt_img, border=border)
